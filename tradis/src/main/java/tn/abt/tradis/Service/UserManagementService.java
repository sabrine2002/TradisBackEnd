...
public class UserManagementService {
    ...
    public long countAgents() {
        return userRepository.countByRole(RoleName.ROLE_AGENT);
    }
    ...
    public List<AgentDTO> getAllAgents() {
        return userRepository.findAll()
                ...
                .filter(user -> user.getRoles()
                        .stream()
                        .anyMatch(role -> role.getName() == RoleName.ROLE_AGENT))
                .map(user -> new AgentDTO(user.getFirstname(), user.getLastname(), user.getEmail(), user.getAgencyCode()))
                .collect(Collectors.toList());
    }
